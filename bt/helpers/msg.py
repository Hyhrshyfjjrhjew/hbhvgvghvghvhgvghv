...
def getChatMsgID(link: str):
    ...
    try:
        if len(linkps) == 7 and linkps[3] == "c":
            # Format: https://t.me/c/channel_id/thread_id/message_id (forum topic)
            chat_id = get_channel_id(int(linkps[4]))
            message_thread_id = int(linkps[5])
            message_id = int(linkps[6])
        elif len(linkps) == 6:
            if linkps[3] == "c":
                # Format: https://t.me/c/channel_id/message_id (regular channel)
                chat_id = get_channel_id(int(linkps[4]))
                message_id = int(linkps[5])
            else:
                # Format: https://t.me/username/thread_id/message_id (forum topic)
                chat_id = linkps[3]
                message_thread_id = int(linkps[4])
                message_id = int(linkps[5])
        elif len(linkps) == 5:
            # Format: https://t.me/username/message_id (regular chat)
            chat_id = linkps[3]
            if chat_id == "m":
...
def message_belongs_to_topic(message, topic_id: int) -> bool:
    ...
    Based on the reference script logic
    """
